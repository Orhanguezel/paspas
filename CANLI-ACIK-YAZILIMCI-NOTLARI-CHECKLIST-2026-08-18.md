...
| Durum | Adet | Anlamı |
|---|---:|---|
| Açık (`open`) | **0** | ✅ 15 Ağustos notlarının tamamı çözüldü |
| Bilgi bekliyor (`needs_info`) | 6 | 4 Web (grafikçi) + 2 stok sorusu (müşteri yanıtı) |
| **Toplam aktif** | **6** | Hiçbiri bizde beklemiyor |
...
- [x] Satırdaki `Sipariş: SS-2026-0059` bilgisini kaldır ([satır 1006-1008](admin_panel/src/app/\(main\)/admin/uretim-emirleri/_components/uretim-emirleri-client.tsx#L1006-L1008)).
- [x] `ÇİFT TARAFLI` rozetini kaldır ([satır 996-1004](admin_panel/src/app/\(main\)/admin/uretim-emirleri/_components/uretim-emirleri-client.tsx#L996-L1004)).
- [x] `+1 taraf daha` yazısını kaldır ([satır 976-980](admin_panel/src/app/\(main\)/admin/uretim-emirleri/_components/uretim-emirleri-client.tsx#L976-L980)).
- [x] İlerleme sütunundaki `0 / 4.100` metnini kaldır, progress bar kalsın ([satır 1052-1055](admin_panel/src/app/\(main\)/admin/uretim-emirleri/_components/uretim-emirleri-client.tsx#L1052-L1055)).
- [x] Kazanılan yer kadar satır yüksekliğini azalt — ekrana daha çok satır sığsın.
- [x] Yeni **Üretilen** sütunu: operatör montaj tarafı için veri girmediyse boş; girdiyse girilen miktarların toplamı + yanında `Takım`.
- [x] Grup başlığındaki üretim emri numarası (sol üst `UP-2026-0036`) satırdaki emir no formatıyla aynı/belirgin olsun; hemen yanına **ürün grubu** yazılsın (örn. `Pars Grubu`, `Ekstra Grubu`).
- [x] Durum etiketi `Üretimde` → `Üretiliyor` — **yalnız** [tr.json:5296](admin_panel/src/locale/tr.json#L5296) (`uretimEmirleri.statuses.uretimde`). [tr.json:5117](admin_panel/src/locale/tr.json#L5117) `satisSiparisleri.statuses.uretimde`'dir ve Satış Siparişleri ekranlarına aittir — **dokunulmayacak** (bkz. Risk R3).

### 1.4 ✅ Üretim emri satırında açılır detay ekranı
...
- [x] İlerleme sütunundaki makine bazlı alt kırılımı (`900 T (ÖN) · Başlamadı` listesi) satırdan kaldır — Görsel1'de mavi işaretli blok ([satır 1060-1088](admin_panel/src/app/\(main\)/admin/uretim-emirleri/_components/uretim-emirleri-client.tsx#L1060-L1088)).
- [x] Her emir satırının başına `>` genişletme işareti koy (ürünler ekranındaki mevcut yapı örnek alınacak — Görsel2).
- [x] Tıklanınca satır altında açılan panel; tasarım Makine İş Yükleri ekranına benzer (Görsel3).
- [x] Panelde o emir numarasına bağlı **üretimler** listelensin (çift taraflı üretimde 2 satır).
- [x] Panel alanları:
  - [x] **Ürün Adı — Operasyonel YM Adı**: Operasyonel YM adı ürün adına göre daha belirgin.
  - [x] Montaj varsa `Montaj` rozeti (Görsel3'teki gibi).
  - [x] **Makine Adı**.
  - [x] **Üretim Durumu**: `Makineye atandı` / `Üretiliyor` / `Duraklatıldı` (+ duraklama nedeni küçük açıklama) / `Tamamlandı`. İkonla desteklenebilir.
  - [x] **Üretilen Miktar**: operatörün girdiği toplam; montajlıysa sonda `Takım`, montajsızsa `Adet`.
  - [x] **Bitiş Tarihi**: tamamlanmadıysa planlanan bitiş `17/08 Pazartesi` + altında saat; tamamlandıysa `17/08/2026` + saat. Planlanan ve gerçekleşen **farklı renkte**.


✅ **CANLIDA — release `20260818T215344Z-8b0c1bbc880d`** (commit `76c55e3` + `8b0c1bb`) · her iki not `resolved`

**Tarayıcıda doğrulandı:** grup başlığı `UP-2026-0036 · PARS Grubu`, satırda `2.000 Takım`, panelde iki üretim
(Sağ · Duraklatıldı · 1.300 Adet · 18/08 Salı 04:36 — Sol · Montaj rozeti · Üretiliyor · 2.000 Takım · 19/08 Çarşamba 21:36).
Kaldırılanlar teyit edildi (sipariş no, ÇİFT TARAFLI, +1 taraf daha). JS hatası yok.

**R1 canlı veriyle sınandı:** 101 mamul grubuna uygulandı. Toplam alsaydık yanlış çıkacak örnekler —
UE-2026-0158 (2.000 yerine 3.300), UE-2026-0003 (3.999 yerine 4.999), UE-2026-0007 (125 yerine 250).

**Doğrulama bir hata yakaladı:** UE-2026-0032'de enjeksiyon 780 üretmiş ama montaj 0 iken sütun "0 Takım"
gösteriyordu. Müşteri isteği "montaj tarafı için veri girilmediyse boş gelsin" olduğundan düzeltildi (`8b0c1bb`);
"0" yazmak üretim yapılmış izlenimi verirdi.

**Eksik bırakılan:** duraklama nedeni açıklaması — `durus_kayitlari` ayrı tabloda, makine kuyruğu üzerinden bağlı.
Müşteri "olabilir" demişti; nota da yazıldı.
...
4. ~~**Paket 4 — 1.3 + 1.4 + R1 + R3 + S4**~~ ✅ **TAMAMLANDI** (18 Ağu): üretim emirleri ekran yenilemesi. Satır sadeleştirme + detay paneli + üç kurallı "Üretilen" hesabı + tek locale anahtarı + aşım gösterimi. En büyük iş kalemi.
