...
import { createSatinAlmaSiparisi, listSatinAlmaSiparisleri, updateSatinAlmaSiparisi } from "../controller";
...
describe("satin_alma validation", () => {
  ...
  it("rejects invalid list filters", () => {
    ...
    expect(listQuerySchema.safeParse({ tedarikciId: "x" }).success).toBe(false);
    expect(listQuerySchema.safeParse({ durum: "beklemede" }).success).toBe(false);
    expect(listQuerySchema.safeParse({ sort: "tedarikci" }).success).toBe(false);
  });

  it("validates create body constraints", () => {
    const valid = createSchema.safeParse({
      siparisNo: "SA-001",
      tedarikciId: "11111111-1111-1111-1111-111111111111",
      siparisTarihi: "2026-03-04",
      items: [{ urunId: "22222222-2222-2222-2222-222222222222", miktar: 10 }],
    });

    expect(valid.success).toBe(true);
    expect(createSchema.safeParse({ siparisNo: "", items: [] }).success).toBe(false);
    expect(
...
describe("satin_alma controller", () => {
  it("returns 400 on invalid list query", async () => {
    ...
    await listSatinAlmaSiparisleri(req, reply);
  ...
  it("returns 400 on invalid create body", async () => {
    ...
    await createSatinAlmaSiparisi(req, reply);
  ...
  it("returns 400 on empty patch body", async () => {
    ...
    await updateSatinAlmaSiparisi(req, reply);
