...
describe("satis_siparisleri validation", () => {
  ...
  it("rejects invalid list filters", () => {
    ...
    expect(listQuerySchema.safeParse({ musteriId: "x" }).success).toBe(false);
    expect(listQuerySchema.safeParse({ durum: "hazirlaniyor" }).success).toBe(false);
    expect(listQuerySchema.safeParse({ sort: "musteri" }).success).toBe(false);
  });

  it("validates create body constraints", () => {
    const valid = createSchema.safeParse({
      siparisNo: "S-001",
      musteriId: "11111111-1111-1111-1111-111111111111",
      siparisTarihi: "2026-03-04",
      durum: "planlandi",
      items: [{ urunId: "22222222-2222-2222-2222-222222222222", miktar: 10 }],
    });

    expect(valid.success).toBe(true);
    expect(createSchema.safeParse({ siparisNo: "S-1", musteriId: "x", siparisTarihi: "2026-03-04", items: [] }).success).toBe(false);
    expect(
      createSchema.safeParse({
        siparisNo: "S-1",
        musteriId: "11111111-1111-1111-1111-111111111111",
        siparisTarihi: "04-03-2026",
  ...
  it("accepts extended sales statuses", () => {
...
describe("satis_siparisleri controller", () => {
  it("returns 400 on invalid list query", async () => {
    ...
    await listSatisSiparisleri(req, reply);
  ...
  it("returns 400 on invalid create body", async () => {
    const req = {
      body: { siparisNo: "", items: [] },
      log: { error: () => {} },
    } as any;
    const reply = createFakeReply() as any;

    await createSatisSiparisi(req, reply);
  ...
  it("returns 400 on empty patch body", async () => {
    ...
    await updateSatisSiparisi(req, reply);
