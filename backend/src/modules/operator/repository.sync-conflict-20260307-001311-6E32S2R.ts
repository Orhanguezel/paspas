...
export async function repoSevkiyatOlustur(
  ...
  await db.transaction(async (tx) => {
    ...
    for (const kalem of body.kalemler) {
      const kalemId = randomUUID();
      await tx.insert(sevkiyatKalemleri).values({
        id: kalemId,
        sevkiyat_id: sevkiyatId,
        musteri_id: kalem.musteriId,
        siparis_id: kalem.siparisId ?? null,
        siparis_kalem_id: kalem.siparisKalemId ?? null,
      ...
      await tx
        .update(urunler)
        .set({ stok: sql`${urunler.stok} - ${kalem.miktar.toFixed(4)}` })
