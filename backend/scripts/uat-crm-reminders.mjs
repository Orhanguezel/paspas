...
finally{await db.execute('DELETE FROM notifications WHERE title=?',[title]);if(reminderId)await db.execute('DELETE FROM crm_reminders WHERE id=?',[reminderId]);await db.execute('DELETE FROM crm_activities WHERE id=?',[activityId]);await db.execute('DELETE FROM musteriler WHERE id=?',[customerId]);await db.end();}
