...
function normalizeSettingRow(row: SiteSettingRow | null): SiteSettingRow | null {
  if (!row || typeof row !== 'object') return row;
  if (typeof row.value !== 'string') return row;

  const raw = row.value.trim();
  if (!raw || (!raw.startsWith('{') && !raw.startsWith('['))) return row;

  try {
    return { ...row, value: JSON.parse(raw) as JsonLike };
  } catch {
    return row;
  }
}
...
export async function fetchSetting(
  ...
  if (!row) return null;
  if (typeof row === 'object' && row && 'data' in (row as any)) {
    return normalizeSettingRow((row as any).data ?? null);
  }
  return normalizeSettingRow(row as SiteSettingRow);
}
