...
import { useListLeadCandidatesQuery } from '@/integrations/hooks';
...
export function NavMain({ items, showQuickCreate = false }: NavMainProps) {
  ...
  const { data: pendingLeadCandidates } = useListLeadCandidatesQuery(
          ...
          <SidebarGroupContent className="flex flex-col gap-2">
            <SidebarMenu>
              {group.items.map((item) => {
                const itemBadgeCount = item.badgeKey === 'notifications_unread'
                  ? (unreadCount?.count ?? 0)
                  : item.badgeKey === 'lead_candidates_pending'
                    ? (pendingLeadCandidates?.length ?? 0)
                    : undefined;
