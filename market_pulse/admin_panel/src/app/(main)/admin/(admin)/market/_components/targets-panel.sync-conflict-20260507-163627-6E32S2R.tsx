...
export default function TargetsPanel() {
  ...
  const [deleteTarget, { isLoading: isDeleting }] = useDeleteMarketTargetMutation();
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-gm-gold font-bold text-[10px] tracking-[0.2em] uppercase">Pazar İstihbaratı</span>
          </div>
      ...
      <Card className="bg-gm-bg-deep/50 border-gm-border-soft rounded-[32px] overflow-hidden backdrop-blur-md shadow-2xl">
        <CardContent className="p-8 grid gap-8 md:grid-cols-2 lg:grid-cols-4 items-end text-gm-text">
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-bold text-gm-muted tracking-[0.2em] uppercase ml-1">Kategori</label>
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-bold text-gm-muted tracking-[0.2em] uppercase ml-1">Durum</label>
          ...
          <Table>
            <TableHeader className="bg-gm-surface/40">
              <TableRow className="border-gm-border-soft hover:bg-transparent">
                <TableHead className="py-6 px-8 text-[10px] font-bold uppercase tracking-widest text-gm-muted">Firma Bilgileri</TableHead>
                <TableHead className="py-6 text-[10px] font-bold uppercase tracking-widest text-gm-muted text-center">Kategori</TableHead>
                <TableHead className="py-6 text-[10px] font-bold uppercase tracking-widest text-gm-muted text-center">Durum</TableHead>
                <TableHead className="py-6 text-[10px] font-bold uppercase tracking-widest text-gm-muted text-center">Şehir</TableHead>
                <TableHead className="py-6 text-[10px] font-bold uppercase tracking-widest text-gm-muted text-right pr-12">Churn Riski</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                data?.map((target) => (
                  <TableRow key={target.id} className="border-gm-border-soft hover:bg-gm-primary/[0.03] transition-colors group">
                    ...
                    <TableCell className="py-6 text-center">
                      {(() => {
                        const cfg = STATUS_CONFIG[target.status] || STATUS_CONFIG.archived;
                        return (
                          <div className={cn(
                    ...
                    <TableCell className="py-6 text-right pr-12">
                      {(() => {
                        const badge = churnBadge(target.churnRiskScore);
                        return (
