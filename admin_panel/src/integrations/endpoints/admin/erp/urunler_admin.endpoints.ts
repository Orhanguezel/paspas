...
export const urunlerAdminApi = baseApi.injectEndpoints({
  endpoints: (b) => ({
    listUrunlerAdmin: b.query<
      UrunListResponse,
      { search?: string; page?: number; limit?: number; kategori?: UrunKategori; tedarikTipi?: TedarikTipi } | void
    >({
    ...
    listOperasyonlarAdmin: b.query<UrunOperasyonDto[], string>({
      ...
      transformResponse: (res: unknown) =>
        Array.isArray(res) ? (res as unknown[]).map(normalizeOperasyon) : [],
    ...
    getUrunReceteAdmin: b.query<ReceteDto | null, string>({
      ...
      transformResponse: (res: unknown) => {
        if (!res || (typeof res === 'object' && 'recete' in (res as Record<string, unknown>) && (res as Record<string, unknown>).recete === null)) {
          return null;
        }
    ...
    saveUrunReceteAdmin: b.mutation<ReceteDto, { urunId: string; items: { urunId: string; miktar: number; fireOrani: number; sira: number }[] }>({
